...
num_of_cells = (2 * Tr + 2 * Gr + 1) * (2 * Td + 2 * Gd + 1) - (2 * Gr + 1) * (2 * Gd + 1);
thresholded_signal = zeros(Nr / 2, Nd);
...
for i = 1:(Nr/2 - (2 * Gr + 2 * Tr))
    for j = 1:(Nd - (2 * Gd + 2 * Td))
        ...
        threshold = noise_level / num_of_cells;      
        ...
        thresholded_signal(i + Tr + Gr,j + Td + Gd) = signal;  
...
figure,surf(doppler_axis, range_axis, thresholded_signal);
colorbar;